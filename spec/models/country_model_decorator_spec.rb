...
describe 'CountryModelDecorator' do
  ...
  describe 'abbr_of_countries_with_cities' do
    it 'returns the country abbreviation of only the countries with registered cities' do
      country = FactoryGirl.create :country, abbr: 'MX'
      state = FactoryGirl.create :state, country: country
      city = FactoryGirl.create :city, state: state
      country_with_no_cities = FactoryGirl.create :country, name: 'Venezuela'
      CountryModelDecorator.abbr_of_countries_with_cities.should == ['MX']
    end
  end
end
